...
# Lazy imports - will be populated when check_voice_dependencies() is called
np = None
sd = None
...
def check_voice_dependencies(required_deps: List[str] = None) -> None:
    ...
    - Import and set global references if all dependencies are available
    """
    global np, sd
    ...
    if missing_deps:
        ...
        print("-"*70)
        print("\nTo install voice dependencies (recommended):")
        print("  1. Navigate to the repository root directory")
        print("  2. Run: pip install -e \".[voice]\"")
        print("\nThis will install:")
        print("  - sounddevice (for audio I/O)")
        print("  - piper-tts (for text-to-speech)")
        print("  - All other voice dependencies")
        print("\nFor detailed installation instructions, see:")
        print("  README.md")
        print("\n" + "="*70)
        sys.exit(1)

    # Import dependencies now that we know they're available
    import numpy
    import sounddevice
    np = numpy
    sd = sounddevice

from hailo_agent.core.defines import REPO_ROOT, TARGET_SR
...
def _ensure_voice_deps():
    """Ensure voice dependencies are loaded before use."""
    global np, sd
...
class AudioDiagnostics:
    ...
    @staticmethod
    def resample_audio(audio, original_sr: float, target_sr: float):
        """
        Resample audio from original_sr to target_sr using numpy linear interpolation.

        Args:
            audio: Input audio data (1D array).
    ...
    @staticmethod
    def test_microphone(device_id: int, duration: float = 1.0, threshold: float = 0.001) -> Tuple[bool, str, float, Optional[any]]:
        """
        ...
        Returns:
            Tuple[bool, str, float, Optional[any]]: (Success, Message, Max Amplitude, Recorded Data)
        """
        _ensure_voice_deps()
        try:
    ...
    @staticmethod
    def test_speaker(device_id: int, duration: float = 1.0, audio_data: Optional[any] = None) -> Tuple[bool, str]:
        """
        ...
        Args:
            ...
            audio_data (Optional[any]): Audio data to play. If None, generates a tone.
